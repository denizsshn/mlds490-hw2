#This is the code where I implemented a little epsilon (0.01) for the evaluation part to prevent the deterministic pit
import torch
...
for i in range(500):
    ...
    while not done:
        action = agent.choose_action(state, EVAL_EPSILON)
...
plt.grid(True)
plt.savefig(f"{ENV_NAME}_mini_q_values_pytorch.png")
plt.close() 
...
plt.legend()
plt.grid(True)
plt.savefig(f"{ENV_NAME}_mini_rewards_pytorch.png")
plt.close() 
...
plt.legend()
plt.grid(True)
plt.savefig(f"{ENV_NAME}_mini_histogram_pytorch.png")
plt.close() 
